...
import toursData from "../../../data/toursData.json" // Новый единый JSON туров
import "../../Tours/Tours.css" // Подключаем стили карточек и модального окна
import "./IssykKul.css"
...
function IssykKul() {
  ...
  const regionInfo = REGIONS_DATA.find(
    (r) => r.id === "issyk-kul" || r.id === "issykKul"
  ) || {
    name: "regions.issykKul",
    img: ""
  }

  // Фильтруем туры для Иссык-Кульской области
  const issykKulTours = toursData.filter(
    (tour) =>
      tour.regions.includes("issykKul") ||
      tour.regions.includes("kyrgyzstan") ||
      tour.regions.includes("all")
  )

  // Получаем программу тура из перевода (работает и с объектом, и с массивом)
      ...
      <div className="region-detail__container">
        ...
        <div className="region-detail__tours-section">
          ...
          <div className="tours-grid">
            {issykKulTours.length > 0 ? (
              issykKulTours.map((tour) => (
