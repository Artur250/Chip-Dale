...
function Naryn() {
  ...
  // Фильтруем туры для Нарынской области + всеобщие туры по Кыргызстану
  const narynTours = toursData.filter(
    (tour) =>
      tour.regions.includes("naryn") ||
      tour.regions.includes("all") ||
      tour.regions.includes("kyrgyzstan") ||
      tour.key === "tour1"
  )

  // Получаем программу тура из перевода (работает с объектом и массивом)
