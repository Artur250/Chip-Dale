...
function JalalAbad() {
  ...
  const regionInfo = REGIONS_DATA.find(
    (r) => r.id === "jalal-abad" || r.id === "jalalAbad"
  ) || {
    name: "regions.jalalAbad",
    img: ""
  }

  // Фильтруем туры для Джалал-Абадской области
  const jalalAbadTours = toursData.filter(
    (tour) =>
      tour.regions.includes("jalalAbad") ||
      tour.regions.includes("jalal-abad") ||
      ...
      <div className="region-detail__container">
        ...
        <div className="region-detail__tours-section">
          ...
          <div className="tours-grid">
            {jalalAbadTours.length > 0 ? (
              jalalAbadTours.map((tour) => (
